...
namespace KMHSSS
{
    public partial class Outstanding_Van_Fees_Report_Frm : Form
    {
        string query, Van = "YES";
        SqlConnection con = new SqlConnection(@"Data Source=.\SQLEXPRESS;Initial Catalog=DBKMHSS;Integrated Security=True");
        public Outstanding_Van_Fees_Report_Frm()
        {
            InitializeComponent();
        }
        ...
        {            
            try
            {
                if (Month_CBox.Text != "")
                {
                    query = "select S.Admission_No,S.Std_Name,S.Class,S.Section,S.SMonth from Student_Admission_Master_Table S where S.SYear='" + SYear_CBox.Text + "' and S.EYear='" + EYear_Txt.Text + "' and S.SMonth='" + SMonth_CBox.Text + "' AND S.Van='" + Van + "' and S.Admission_No not in(select B.Admission_No from Van_Bill_Master_Table B where B.Month='" + Month_CBox.Text + "' and B.SYear='" + SYear_CBox.Text + "' and B.EYear='" + EYear_Txt.Text + "') order by S.Class,S.Section,S.Gender Asc";
                ...
                {
                    MessageBox.Show("Plz Choose Any Month");
                }
            }
            catch (Exception Ex)
            {
        ...
        private void Outstanding_Van_Fees_Report_Frm_Load(object sender, EventArgs e)
        {
            try
            {
                fill_AYear_CBox();
