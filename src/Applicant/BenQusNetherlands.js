...
class BenQusNetherlands extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        if (Params != null || Params != "") {
            if (Mode != null || Mode != "") {
                if (Mode == "E") {
                    this.handleBenQusNetherlandsEdit(Benefit);
                }
                else {
                    this.handleBenQusNetherlandsAuto(this);
                    this.handleResNetherlandsAuto(this);
                }
            }
            else {
                this.handleBenQusNetherlandsAuto(this);
    ...
    handleResNetherlandsAuto(event) {
        var thisObj = this;
        let UserID;
    ...
    handleBenQusNetherlandsAuto(event) {
        var thisObj = this;
        let UserID;
        //let BenQusAutoAPIUrl = "https://5zykex8q94.execute-api.us-west-2.amazonaws.com/Dev/GPA_AutopopulateData_Lambda";
        let BenQusAutoAPIUrl = "https://ch06is45h0.execute-api.us-west-2.amazonaws.com/Dev/";
            ...
            <div >
                <Col xs={12} md={12} style={newstyle}>
                    <Panel eventKey="1" defaultExpanded={true}>
                        <Panel.Heading>
                            <Panel.Title >Netherlands Form</Panel.Title>
                        </Panel.Heading>
                                ...
                                <Col xs={12} md={12} className="PanelText">
                                    <Col xs={12} md={12} >
                                        <Col xs={12} md={12} className="input-fileds align-fileds">
                                            <label>I apply for all benefits for which I am eligible under the provisions of the social security agreement between the United States and Netherlands</label>
                                        </Col>
                                    </Col>
                                    <Col xs={12} md={12} >
    ...
    // Edit function
    handleBenQusNetherlandsEdit(event) {
    ...
    handlePensionDocId(e) {
        ...
        let DocIDData = {
            QueryName: "NetherlandPensionForm",
            UserID: emailresult,
        };
        SaveDataAPICallMailSend(Docurl, DocIDData)
            .then((data) => {
                console.log("Successfully generated document ID", data);
                this.handleAppDocTrackEntry(data[0].NetherlandPensionEmpID, data[0].DocumentID);
            }).catch((err) => {
                console.log(err);
            });
    }

    handleAppDocTrackEntry(NetherlandsPensionID, DocID) {
        let Docurl = " https://unvzh1qti4.execute-api.us-west-2.amazonaws.com/Dev/GPA_AdminModule_Lambda";
        let DocIDData = {
            QueryName: "PensionFormAppDocTrackingInsert",
            UserID: emailresult,//"spurthi.n@mitosistech.com",            
            DocumentID: DocID,
            DocumentCode: NetherlandsPensionID,
            CountryCode: "NL",//this.state.CountryCode,            
            SendDate: new Date(),
...
export default connect(mapReducerStateToProps, mapDispatchToProps)(BenQusNetherlands);