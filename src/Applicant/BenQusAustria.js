...
class BenQusAustria extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        if (Params != null || Params != "") {
            if (Mode != null || Mode != "") {
                if (Mode == "E") {
                    this.handleBenQusAustriaEdit(Benefit);
                }
                else {
                    this.handleBenQusAustriaAuto(this);
                    this.handleResAustriaAuto(this);
                }
            }
            else {
                this.handleBenQusAustriaAuto(this);
            ...
            <div >
                <Col xs={12} md={12} style={newstyle}>
                    <Panel eventKey="1" defaultExpanded={true}>
                        <Panel.Heading>
                            <Panel.Title >Austria Form</Panel.Title>
                        </Panel.Heading>
                                ...
                                <Col xs={12} md={12} className="PanelText">
                                    <Col xs={12} md={12} >
                                        <Col xs={12} md={12} className="input-fileds align-fileds">
                                            <label>I apply for all benefits for which I am eligible under the provisions of the social security agreement between the United States and Austria</label>
                                        </Col>
                                    </Col>
                                    <Col xs={12} md={12} >
    ...
    handlePensionDocId(e) {
        ...
        let DocIDData = {
            QueryName: "AustriaPensionForm",
            UserID: emailresult,
        };
        SaveDataAPICallMailSend(Docurl, DocIDData)
            .then((data) => {
                console.log("Successfully generated document ID", data);
                this.handleAppDocTrackEntry(data[0].AustriaPensionEmpID, data[0].DocumentID);
            }).catch((err) => {
                console.log(err);
            });
    }

    handleAppDocTrackEntry(AustriaPensionID, DocID) {
        let Docurl = " https://unvzh1qti4.execute-api.us-west-2.amazonaws.com/Dev/GPA_AdminModule_Lambda";
        let DocIDData = {
            QueryName: "PensionFormAppDocTrackingInsert",
            UserID: emailresult,//"spurthi.n@mitosistech.com",            
            DocumentID: DocID,
            DocumentCode: AustriaPensionID,
            CountryCode: "AT",//this.state.CountryCode,            
            SendDate: new Date(),
...
export default connect(mapReducerStateToProps, mapDispatchToProps)(BenQusAustria);